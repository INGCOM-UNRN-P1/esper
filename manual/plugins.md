---
title: "Guía de Extensión y Creación de Plugins: esper"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de esper"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-esper-plugins)=
# Guía de Extensión y Plugins: esper

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`esper`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-esper-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`esper` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para esper`.
- **Punto de Entrada Oficial**: `esper.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-esper-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para esper."""
    name = "custom_rule"
...
Para que `esper` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."esper.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `esper` la reconozca:
...
(manual-esper-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `esper` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = esper.ejecutar_analisis(
...
(manual-esper-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `esper` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar esper
        run: pip install -e ./esper
        
      - name: Ejecutar Auditoría
        run: esper check src/ include/ --json > reporte.json
````

---

(manual-esper-plugins-ejercicios)=
